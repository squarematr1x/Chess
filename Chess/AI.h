...
public:
	...
	int minMax(int depth, int alpha, int beta, MinMaxPlayer player, std::vector<Piece*>& pieces1, std::vector<Piece*>& pieces2, Board& board);
	int evaluate(std::vector<Piece*>& pieces, MinMaxPlayer player, Board& board);
	int min(int a, int b);
	int max(int a, int b);
